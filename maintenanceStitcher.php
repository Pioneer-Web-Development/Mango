...
<html>
<head>
<title>Stitcher Maintenance</title>
<?php
...
<div style='width:940px;height:60px;border-bottom:8px solid #AC1D23;padding-bottom:0px;margin-bottom: 0px;'>
    ...
    <?php
        ...
        ?>
        <input type='button' onClick='document.location.href="?ticketonly=true&equipmentid=0&componentid=0";' value='Submit General Ticket'>
    ...
    if ($_GET['equipmentid'] || $_GET['ticketonly'])
      {
          ...
          if (isset($_GET['stitcherid']))
          {
              $stitcherid=$_GET['stitcherid'];
          } else {
              $stitcherid=$GLOBALS['defaultStitcher'];
          }
          $sql="SELECT id,stitcher_name FROM stitchers";
          $dbStitcher=dbselectmulti($sql);
          $stitchers=array();
          if($dbStitcher['numrows']>1)
          {
              foreach($dbStitcher['data'] as $i)
              {
                $stitchers[$i['id']]=$i['stitcher_name'];    
              }
              print "Please select an stitcher: ".input_select('stitcherid',$stitchers[$stitcherid],$stitchers,'',"chgInserter(this.value)");
          }
          
          build_stitcher($stitcherid);
          print "</div>\n";
...
{
    global $productionStaff,$generalProductionTicketType,$siteID, $defaultStitcher;
   ...
   print "<div id='partsreplace' style='float:left;margin-left:10px;width:830px;height:600px;overflow:hidden;'>\n";
      ...
      print "</div><!-- closes the history tab -->\n";
       } // closes ticket only conditional  
...
function build_stitcher($stitcherid)
{
      ...
      if ($dbPockets['numrows']>0)
      {
          ...
          print "<div id='stitcherholder' style='margin-bottom:20px;'>\n";
          foreach($dbPockets['data'] as $pocket)
          {
             ...
             $(document).ready(function(){
                 $('#station_<?php echo $pocket['id']?>').click(function(){
                     window.location="maintenanceStitcher.php?type=stitcher&equipmentid=<?php echo $stitcherid ?>&componentid=<?php echo $pocket['id']; ?>";    
                 })
             })
             </script>
             <?php
