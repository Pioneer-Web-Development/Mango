...
/*
* NEW FUNCTIONALITY ADDED BY JOE HANSEN ON 12/24/12
* This functionality is designed to allow a user to delete an uploaded file
* 
* add to _onComplete in the result.success test 
*   this._find(item, 'trash').style.display = "inline-block";
* 
* around 1652, when extending the basic uploader
* trash: function(fileId,fileName) {
        var trashDir=this._paramsStore.getParams(fileId).dir;
        var item = this.getItemByFileId(fileId);
        var trashName=this._handler.getName(fileId);
        if(trashName!='')
        {
            $.ajax({
               url: 'includes/ajax_handlers/fineUploaderHandler.php',
               type: "POST",
               data: {action:'delete',path:trashDir,filename:trashName},
               dataType: 'json',
               success: function(response) {
                   if(response.status=='success')
                   {
                      qq(item).remove();
                      qq.log('deleted file '+trashName+' from '+trashDir);
                   } else {
                      alertMessage("Job deletion failed<br />"+response.status+'<br>'+response.message,'error');
                   }
               }
           });
        }
    ...
    around 624 add to callbacks:
    
    onTrash: function(id, fileName){},
...
qq.FineUploaderBasic.prototype = {
    ...
    trash: function(fileId,fileName) {
        this._handler.trash(fileId,fileName);
    },
    reset: function() {
    ...
    _createUploadHandler: function(){
        ...
        var handler = new qq[handlerClass]({
            ...
            onTrash: function(id, fileName){
                self._onTrash(id, fileName);
                self._options.callbacks.onTrash(id, fileName);
            },
            onUpload: function(id, fileName, xhr){
...
qq.FineUploader = function(o){
    ...
    qq.extend(this._options, {
        ...
        fileTemplate: '<li>' +
            ...
            '<a class="qq-upload-trash" href="#">&nbsp;</a>' +
...
qq.extend(qq.FineUploader.prototype, {
    ...
    _bindTrashEvents: function(){
        ...
        this._disposeSupport.attach(list, 'click', function(e){
            ...
            if (qq(target).hasClass(self._classes.trash)){
                qq.preventDefault(e);
...
qq.extend(qq.UploadHandlerForm.prototype, {
    ...
    _trash: function(id){
        this._options.onTrash(id, this.getName(id));
...
qq.extend(qq.UploadHandlerXhr.prototype, {
     ...
     * Returns id to use with upload, cancel & trash
     **/
    add: function(file){
