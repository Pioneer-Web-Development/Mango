		...
		for (i=0; i<segCnt; i++) {
			...
			if (levelI) {
				...
				if (forward) {
				    ...
				    outerWidth = availWidth / (forward + 1);  //testing to see if we can eliminate overlapping events
                }else{
			...
			seg.top = top;
			seg.left = left;
			//seg.outerWidth = outerWidth;  //testing to see if we can eliminate overlapping events
...
function View(element, calendar, viewName) {
	...
	function eventDrop(e, event, dayDelta, minuteDelta,  ev, ui) {
		var oldAllDay = event.allDay;
		var eventId = event._id;
		moveEvents(eventsByID[eventId], dayDelta, minuteDelta);
		trigger('eventDrop', e, event, dayDelta, minuteDelta, function() {
