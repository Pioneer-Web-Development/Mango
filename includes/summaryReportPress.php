...
if ($_POST)
{
    ...
    print "<a href='summaryReportPress.php'>Run another report</a><br />\n";
    $pubid=$_POST['pub_id'];
    ...
    if ($pubid==0)
    {
        $sql="SELECT A.* FROM jobs A, job_stats B WHERE A.site_id=$siteID AND A.stats_id=B.id AND B.startdatetime_actual>='$startdate' AND B.stopdatetime_actual<='$enddate' AND A.pub_date<>'' AND A.continue_id=0 ORDER BY A.pub_id, A.pub_date";
    } else {
        $sql="SELECT A.* FROM jobs A, job_stats B WHERE A.site_id=$siteID AND A.pub_id=$pubid AND A.stats_id=B.id AND B.startdatetime_actual>='$startdate' AND B.stopdatetime_actual<='$enddate' AND A.pub_date<>'' AND A.continue_id=0 ORDER BY A.pub_id, A.pub_date";
    }
