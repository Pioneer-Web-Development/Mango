...
if ($_POST['submit']=='To Excel')
{
    ...
    if ($_POST['vendor']==0)
    {
        foreach($vendors as $vid=>$vname)
        {
            if ($vid!=0)
            {
                $vrolls=vendor_rolls($vid,$vname,$startdate,$enddate,$source,$psize,$ptype);
        ...
        $vrolls=vendor_rolls($_POST['vendor'],$vendors[$_POST['vendor']],$startdate,$enddate,$source,$psize,$ptype);
        $totalreceivecount+=$vrolls['vtrcount'];
        $totalreceiveweight+=$vrolls['vtrweight'];
        $totalconsumecount+=$vrolls['vtccount'];
        $totalconsumeweight+=$vrolls['vtcweight'];
...
function vendor_rolls($vendorid,$vendorname,$startdate,$enddate,$source='',$size='',$type='',$excel=false)
{
    global $siteID;
    ...
    if ($dbOrders['numrows']>0)
    {
       ...
       foreach($dbOrders['data'] as $order)
       {
            ...
            WHERE status<>99 AND order_id=$order[id] AND receive_datetime>='$startdate' AND receive_datetime<='$enddate' $size $type
             ORDER BY receive_datetime DESC";
            //print "Pulling manifests with $sql<br>";
            ...
            if ($dbReceiveManifests['numrows']>0)
            {
                foreach($dbReceiveManifests['data'] as $rmanifest)
                {
                    ...
                    $sql="SELECT DISTINCT(common_name) as paper, roll_width as width, count(id) as rollcount, sum(roll_weight) as totalweight, order_item_id FROM rolls WHERE status<>99 AND order_id=$order[id] AND manifest_number='$manifest' $size GROUP BY common_name, roll_width ORDER BY width ASC";
                    ...
                    if ($dbRollTotal['numrows']>0)
                    {
                        foreach ($dbRollTotal['data'] as $rolltotal)
                        {
                            ...
                            $sql="SELECT count(id) as rollcount, sum(roll_weight) as totalweight FROM rolls WHERE order_id=$order[id] AND order_item_id=$orderitemid AND manifest_number='$manifest' AND batch_date>='$startdate' AND batch_date<='$enddate' $type $size";
            ...
            $sql="SELECT DISTINCT(manifest_number), receive_datetime FROM rolls 
            WHERE status<>99 AND order_id=$order[id] AND (receive_datetime<='$startdate' OR receive_datetime>='$enddate')
            ...
            if ($dbConsumedManifests['numrows']>0)
            {
               ...
               $exceldata.="</Row>";
                ...
                foreach($dbConsumedManifests['data'] as $cmanifest)
                {
                    ...
                    //now look up the rolls
                    $sql="SELECT DISTINCT(common_name) as paper, order_item_id, roll_width as width, count(id) as rollcount, sum(roll_weight) as totalweight 
                    FROM rolls WHERE status<>99 AND order_id=$order[id] AND manifest_number='$manifest' $type $size GROUP BY common_name, roll_width ORDER BY width ASC";
                    $dbRollTotal=dbselectmulti($sql);
                    ...
                    if ($dbRollTotal['numrows']>0)
                    {
                        foreach ($dbRollTotal['data'] as $rolltotal)
                        {
                            ...
                            AND order_item_id=$orderitemid AND manifest_number='$manifest' AND batch_date>='$bstartdate' AND batch_date<='$benddate' AND common_name='$name'
