...
function setup_topic($action)
{
    ...
    if ($action=='add' || $action=='edit')
    {
        if ($action=='add')
        {
            ...
            $button="Update Topic";
            $id=$_GET['id'];
            $sql="SELECT * FROM helpdesk_topics WHERE id=$id";
        ...
        $id=intval($_GET['id']);
        $sql="DELETE FROM helpdesk_topics WHERE id=$id";
...
function save_topic($action)
{
    ...
    if ($action=='insert')
    {
        $sql="INSERT INTO helpdesk_topics (topic_name, site_id) VALUES ('$name', '$siteID')";
        $dbInsert=dbinsertquery($sql);
        $error=$dbInsert['error'];
    } else {
        $sql="UPDATE helpdesk_topics SET topic_name='$name' WHERE id=$id";
        $dbUpdate=dbexecutequery($sql);
        $error=$dbUpdate['error'];
    }
    if ($error!='')
