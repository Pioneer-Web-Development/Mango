...
function solutions($action)
{
    ...
    if ($action=='add' || $action=='edit')
    {
        if ($action=='add')
        {
            ...
            $solutionid=intval($_GET['solutionid']);
            $sql="SELECT * FROM maintenance_solutions WHERE id=$solutionid";
    ...
    } elseif ($action=='delete')
    {
        ...
        $sql="DELETE FROM maintenance_solutions WHERE id=$solutionid";
        ...
        $sql="SELECT * FROM maintenance_solutions WHERE site_id=$siteID AND parentid=$parentid ORDER BY weight, title";
        $dbSolutions=dbselectmulti($sql);
        //if the parentid!=0, get the parent of the parent
        $sql="SELECT parentid FROM maintenance_solutions WHERE site_id=$siteID AND id=$parentid";
        ...
        if ($dbSolutions['numrows']>0)
        {
            foreach ($dbSolutions['data'] as $solution)
            {
                ...
                $sql="SELECT * FROM maintenance_solutions WHERE parentid=$solution[id]";
...
function save_solution($action)
{
    ...
    if ($action=='insert')
    {
       $sql="INSERT INTO maintenance_solutions (parentid,title,keywords,solution_text,solution_brief, public, site_id, weight) VALUES 
       ('$parentid', '$title', '$keywords', '$solutiontext', '$solutionbrief', '$public', '$siteID', '$weight')";
       $dbInsert=dbinsertquery($sql);
       $error=$dbInsert['error'];
       $solutionid=$dbInsert['numrows']; 
    } else {
       $sql="UPDATE maintenance_solutions SET title='$title', keywords='$keywords', solution_text='$solutiontext', solution_brief='$solutionbrief', weight='$weight', public='$public' WHERE id=$solutionid";
...
function images($action)
{
    ...
    if ($action=='add' || $action=='edit')
    {
        if ($action=='edit')
        {
            $sql="SELECT * FROM maintenance_solutions_images WHERE id=$imageid";
    ...
    } elseif ($action=='delete')
    {
        ...
        if (file_exists($image['path'].$image['filename']))
        {
            ...
            {
                $sql="DELETE FROM maintenance_solutions_images WHERE id=$imageid";
                $dbDelete=dbexecutequery($sql);
                $error=$dbDelete['error'];
                if ($error!='')
                {
        ...
        $sql="SELECT * FROM maintenance_solutions_images WHERE solution_id=$solutionid";
...
function save_image($action)
{
    ...
    if ($action=='insert')
    {
        $sql="INSERT INTO maintenance_solutions_images (caption,solution_id) VALUES ('$caption', '$solutionid')";
        ...
        $sql="UPDATE maintenance_solutions_images SET caption='$caption' WHERE id=$imageid";
        ...
        foreach($_FILES as $file) {
            switch($file['error']) {
                ...
                if($file['name'] != NULL && okFileType($file['type'],'image',$file['name']) != false)  {
                   ...
                   // process the file
                    $date=date("Ym");
                    $path="artwork/maintenanceImages/$date/";
                    if (!file_exists($path))
                    {
                        mkdir($path);
                    }
                    ...
                    if(processFile($file,$path,$newname) == true) {
                        $sql="UPDATE maintenance_solutions_images SET path='artwork/maintenanceImages/$date/', filename='$newname' WHERE id=$imageid";
...
function topics()
{
    ...
    if ($dbGroups['numrows']>0)
    {
        ...
        foreach($dbGroup['data'] as $topic)
        {
            ...
            $sql="SELECT * FROM maintenance_solution_topic_xref WHERE topic_id=$topic[id] AND solution_id=$solutionid";
...
function save_topics()
{
    ...
    $values="";
    //clear existing
    $sql="DELETE FROM maintenance_solution_topic_xref WHERE solution_id=$solutionid";
    $dbDelete=dbexecutequery($sql);
    ...
    $sql="INSERT INTO maintenance_solution_topic_xref (solution_id, topic_id) VALUES $values";
    $dbInsert=dbinsertquery($sql);
    $error=$dbInsert['error'];
    if ($error!='')
    {
...
function changeParent()
{
    ...
    $parentid=$_GET['parentid'];
    $solutionid=$_GET['solutionid'];
    $sql="SELECT id, title FROM maintenance_solutions WHERE parentid=0 AND site_id=$siteID ORDER BY weight";
...
function save_move()
{
    ...
    $sql="UPDATE maintenance_solutions SET parentid=$newparent WHERE id=$solutionid";
    $dbUpdate=dbexecutequery($sql);
